...
bitset<32> f(bitset<32> R, bitset<48> k)
{
    ...
    // преобразование каждого 6-битового блока expandR в 4-битовый
    bitset<32> output;
...
// генерация ключей ki
std::array<bitset<48>,16> generateKeys(bitset<56> originalKey)
{
    bitset<56> key;
    for(int i = 0; i < 56; i ++)
        key[i] = originalKey[B[i]-1];

    std::array<int,28> left, right;
    for(int i = 0; i < 28; i++)
    {
        left[i] = key[i];
        right[i] = key[i+28];
    ...
    std::array<bitset<48>,16> result;

    for(int i = 0; i < 16; i++) // генерируем 16 ключей
    {
        // 1й левый циклический сдвиг
        std::rotate(left.begin(), left.begin() + 1, left.end());
        std::rotate(right.begin(), right.begin() + 1, right.end());
        if(S[i] == 2) // 2й левый циклический сдвиг
        {
            std::rotate(left.begin(), left.begin() + 1, left.end());
            std::rotate(right.begin(), right.begin() + 1, right.end());
        }

        //LR - соединение left и  right
        bitset<56> LR;
        for(int j = 0; j < 28; j++)
        {
            LR[j] = left[j];
...
bitset<64> algorythm(bitset<64> data, std::array<bitset<48>,16> keys, bool flag) // flag = false - encrypt; true - decrypt
{
...
int main()
{
    std::ifstream input( "../archive.tar.gz", std::ios::binary );

    // copies all data into buffer
    std::vector<unsigned char> buffer(std::istreambuf_iterator<char>(input), {});

    bitset<56> key(1859312);
    std::array<bitset<48>,16> keys = generateKeys(key);
    int added_count = 0;
    int blocks_num = buffer.size()/8; // 64 бита = 8 символов из входного потока
    if(buffer.size()%8)
        blocks_num++;
    std::vector<bitset<64>> blocks;
    blocks.resize(blocks_num);
    for(int i = 0; i < blocks_num; i++) // for each 64-bit (8-char) block
    {
        bitset<8> subblock;
        for(int j = 0; j < 8; j++) // for each char in buffer
        {
    ...
    // шифрование
    std::vector<bitset<64>> encrypted;
    ...
    for(int i = 0; i < blocks_num; i++)
    {
        std::cout << "\n";
        for(int j = 0; j < 64; j++)
            std::cout << encrypted[i][j] << " ";
        std::cout << i;
    }

    // расшифровка
    std::vector<bitset<64>> decrypted;
    ...
    for(int i = 0; i < blocks_num; i++)
    {
        std::cout << "\n";
        for(int j = 0; j < 64; j++)
            std::cout << decrypted[i][j] << " ";
        std::cout << i;
    }

    std::ofstream output ( "../archive2.tar.gz", std::ios::binary );
    for(int i = 0; i < blocks_num; i++)
    {
        bitset<8> tmp;
